        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <div className="lg:w-1/2 relative">
            <div className="relative rounded-[2rem] overflow-hidden shadow-2xl">
              <img src={IMAGES.clinicInterior} alt="Recepção DaleSaúde" className="w-full h-auto object-cover transform hover:scale-105 transition-transform duration-700" />
          ...
          <div className="lg:w-1/2">
            <h2 className="text-3xl md:text-5xl font-serif text-dale-blue mb-8">DaleSaúde: saúde que cabe na sua rotina</h2>
            <div className="space-y-6 text-gray-600 leading-relaxed text-lg mb-10">
              <p>Na Tijuca, a <strong>DaleSaúde</strong> foi criada para quem precisa de <strong>atendimento médico de qualidade</strong>, mas não pode perder tempo com filas.</p>
              <p>Reunimos diversas especialidades médicas e exames no mesmo lugar.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
               {[
                 { icon: Heart, text: "Atendimento humanizado" },
                 { icon: Star, text: "Nota 4.9 no Google" },
                 { icon: CalendarCheck, text: "Agendamento rápido" },
                 { icon: Wallet, text: "Preço justo" },
               ].map((item, idx) => (
                 <div key={idx} className="bg-white p-4 rounded-xl shadow-sm border border-sand-200 flex items-center gap-3">
                   <div className="w-10 h-10 rounded-full bg-dale-light flex items-center justify-center text-dale-green"><item.icon size={20} /></div>
